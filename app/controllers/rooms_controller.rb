class RoomsController < ApplicationController
  ...
  before_action :authenticate_manager!, only: [:freelancers_list, :asign_freelancer]
  ...
  def accept
    ...
    if @room.in_progress_freelancers.blank? && freelancer_room.present? && freelancer_room.status == 'pending'
      ...
      @message = Message.new({ body: "Good news, we assigned our expert, <strong>#{current_freelancer.first_name}</strong> to this work. They are rated <strong>4.8 stars</strong> and considered as one of the top talent from Kriya's network. They should be here shortly." })
      @message.room = @room
      ...
      UserNotifierMailer.delay(queue: :room).notify_client_on_new_freelancer(@room.id)
    end
  ...
  def create_room_from_sign_up
    ...
    if @room
      ...
      @room.messages.new({:body => 'Create Project', :room => @room, :user => @room.user})
