...
class Room < ApplicationRecord
  ...
  def reset
    description = self.posts.first

    if description.present?
      self.messages.where('id > ?', description.message.id).destroy_all
      return true
    end
  ...
  def get_room_name_for_user(user, index = nil)
    ...
    elsif !posts.first.nil?
      "#{self.category_name.capitalize}: #{posts.first.title}"
    else
  ...
  def get_room_name_for_freelancer(freelancer, index = nil)
    post = posts.first
    return '' if post.nil?
    "#{self.category_name.capitalize}: #{post.title}".parameterize
  end

  def get_room_name
    post = posts.first
    return '' if post.nil?
    "#{self.category_name.capitalize}: #{post.title}"
  end
