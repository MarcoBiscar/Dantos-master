...
class UserNotifierMailer < ApplicationMailer
  ...
  def notify_welcome_email(user)
    ...
    mail(:from => FROM_RAVI, :to => user.email, :subject => 'Kriya - Important info')
  ...
  def notify_client_on_new_freelancer(room_id)
    @room = Room.find_by(id: room_id)

    return if @room.nil?

    @sendgrid_category = "Room #{room_id}"
    @user = @room.user
    @freelancer = @room.in_progress_freelancers.first
    mail(:from => FROM_GREG, :to => @user.email, :subject => "#{@room.title} (Talent assigned)")
  end
